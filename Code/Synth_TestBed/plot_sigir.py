...
def plot_mse_stderr(PKL, approach_list, plot_estimate, color_string = 'bmgrcky'):
    ...
    for approachIndex, approach in enumerate(approach_list):
        ...
        if plot_estimate:
            ...
            if isinstance(snipPoint, np.ndarray):
                values = PKL[3][approachDict[approach],0:trials,snipPoint.item():]
            else:
        ...
        if isinstance(snipPoint, np.ndarray):
            plt.plot(x[snipPoint.item():], mu, color=currColor, rasterized=True, linewidth=2.0)
        else:
